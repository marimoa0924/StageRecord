...
export default function PostCard({ post, isOwner, onDelete }: Props) {
  ...
  return (
    <Link href={`/post/${post.id}`} className="block">
      <article className="feed-card flex gap-3 px-4 py-4 border-b border-zinc-800/60 cursor-pointer hover:bg-white/[0.02]">
        {/* Avatar */}
        <div className="w-10 h-10 rounded-full bg-sky-500 flex items-center justify-center text-white text-lg shrink-0 mt-0.5 shadow-sm">
          🎭
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* Title */}
          <p className="text-[#e7e9ea] font-semibold text-[15px] leading-snug mt-0.5">{post.title}</p>
          ...
          <div className="mt-3 flex items-center gap-5">
            ...
            <button
              onClick={handleLike}
              className={`flex items-center gap-1.5 text-[13px] transition -my-2 py-2 group ${liked ? 'text-pink-500' : 'text-zinc-500 hover:text-pink-400'}`}
            >
              {liked ? (
